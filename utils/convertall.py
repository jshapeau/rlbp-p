...
def overlay_images_with_color_change(
    ...
    # Resize overlay image to match base image if needed
    ...
    # Convert overlay image to numpy array for manipulation
    ...
    # Paste the modified overlay on top
    result.paste(modified_overlay, (0, 0), modified_overlay)

    # Save the result - keep the RGBA mode to preserve transparency
...
def generate_category_structures(colormap=None):
    ...
    # Define the categories and which class IDs belong to each
    category_mapping = {
    ...
    # Define the "base" class ID for each category (used for main category color)
    category_base_ids = {
        "Bog": 8,  # Treed Bog
...
def generate_all_versions(
    ...
    # Dictionary to store paths of all generated images
    output_paths = {}

    # 0: Base map with just the colormap
...
if __name__ == "__main__":
    ...
    overlay_image_path = "assets/mine.png"
